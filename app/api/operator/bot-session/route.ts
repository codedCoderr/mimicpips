...
async function validateBotBaseUrl(
  baseUrl: string
): Promise<
  { ok: true; url: URL } | { ok: false; error: string; status: number }
> {
  let url: URL;
  try {
    url = new URL(baseUrl);
    ...
    return {
      ok: false,
      error: "Server address is not a valid URL.",
      status: 400,
    };
  }
  ...
  if (url.username || url.password || url.hash) {
    return {
      ok: false,
      error: "Server address cannot include credentials or a fragment.",
      status: 400,
    };
  }
  ...
  if (!allowHttp && url.protocol !== "https:") {
    return {
      ok: false,
      error: "Server address must use https:// in production.",
      status: 400,
    };
  }

  const allowPrivate =
    process.env.ALLOW_PRIVATE_BOT_URLS === "true" ||
    process.env.NODE_ENV !== "production";
  if (!allowPrivate) {
    ...
    if (addresses.length === 0) {
      return {
        ok: false,
        error: "Could not resolve the bot server address.",
        status: 400,
      };
    }
    if (addresses.some(({ address }) => isPrivateAddress(address))) {
      return {
        ok: false,
        error:
          "Bot server address resolves to a private or local network address.",
        status: 400,
      };
    }
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!baseUrl || !apiKey) {
    return NextResponse.json(
      { error: "Server address and API key are required." },
      { status: 400 }
    );
  }
  ...
  if (!upstream.ok) {
    return NextResponse.json(
      { error: `Request failed (${upstream.status}).` },
      { status: 502 }
    );
  }

  const sessionToken = await createOperatorBotSessionToken({
